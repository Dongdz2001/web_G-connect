...
namespace App.Api.Controllers.QTHT
{
    ...
    /// QTHT - đơn vị
    /// </summary>
    [Route("api/qtht-chuc-vu")]
    //phongnhxxx [AuthorizeApi]
    [ApiController]
    public class QTHT_PhieuDeXuatController : ControllerBase
    {
        private readonly IChucVuService _service;
        private readonly INhatKyHeThongService _nhatkyhethongservice;
        public QTHT_PhieuDeXuatController(IChucVuService service,
        ...
        public async Task<ActionResult> Get(int page = 1, int page_size = 0, string sort = null, string filter = null, string search = null)
        {
            try
            {
                var result = await _service.GetManyAsync<ChucVuDTO>(page, page_size, sort, filter, search);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> GetById(Guid id)
        {
            try
            {
                var result = await _service.GetByIdAsync<ChucVuDTO>(id);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<ActionResult> Create([FromBody] ChucVuDTO obj)
        {
            try
            {
                var result = await _service.CreateAsync<ChucVuDTO>(obj);
                ...
                try
                {
                    if (result != null)
                    {
                        ...
                        nkht.duong_dan = "/chuc-vu-view/" + result.id.ToString();
                        nkht.hanh_dong = "208";
        ...
        public async Task<ActionResult> Update([FromBody] ChucVuDTO obj, Guid id)
        {
            try
            {
                var result = await _service.UpdateAsync<ChucVuDTO>(obj, id);
