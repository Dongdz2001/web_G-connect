...
{
    public class qtht_cau_hinh_maService : GenericService<cau_hinh_ma>, qtht_cau_hinh_maIService
    {
        //private readonly IFileDinhKemService _fileSV;
        public qtht_cau_hinh_maService(APPContext dbContext)
           : base(dbContext)
        {
            ...
            _mapperCfg = new MapperConfiguration(cfg =>
            {
                ...
                cfg.AddProfile<qtht_cau_hinh_maProfile>();
                //cfg.CreateMap<IDataRecord, qtht_cau_hinh_maDTO>();
            });
        ...
        protected override IQueryable<cau_hinh_ma> QueryBuilder(IQueryable<cau_hinh_ma> query, dynamic filter, string search)
        {
            if (filter != null)
            {
                ...
                string gia_tri = filter.gia_tri + "";
                if (!string.IsNullOrEmpty(gia_tri))
                {
                    gia_tri = gia_tri.ToLower().Trim();
                    query = query.Where(x => x.gia_tri.ToLower().Contains(gia_tri));
                }
            ...
            if (search != null && search != "")
            {
                ...
                query = query.Where(x => false
                    ...
                    || x.gia_tri.ToLower().Contains(search) 
                );
            }
            return query;
        }
        protected override void BeforeMapper<TDto>(bool isNew, ref TDto dto)
        {
            ...
            if (typeof(TDto) == typeof(qtht_cau_hinh_maDTO))
            {
                qtht_cau_hinh_maDTO qtht_cau_hinh_maDTO = (qtht_cau_hinh_maDTO)(object)dto;
                //xu ly tai day
                //var checkExist = _repo.Where(x => x.ma.ToLower() == qtht_cau_hinh_maDTO.ma.ToLower() && x.id != qtht_cau_hinh_maDTO.id);
