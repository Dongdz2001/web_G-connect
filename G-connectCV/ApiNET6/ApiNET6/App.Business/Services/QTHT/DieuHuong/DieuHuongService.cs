...
{
    public class DieuHuongService : GenericService<dieu_huong>, IDieuHuongService
    {
        public DieuHuongService(APPContext dbContext)
           : base(dbContext)
        {
        ...
        protected override void BeforeAddOrUpdate<TDto>(bool isNew, ref TDto dto, ref dieu_huong entity)
        {
            ...
            var stt_order_cha = _dbContext.dieu_huong.Where(x => x.id == dieuhuongDTO.dieu_huong_cap_tren_id).Select(y => y.stt_order).FirstOrDefault();
            entity.stt_order = stt_order_cha + ((stt_order_cha == "" || stt_order_cha == null) ? "" : ".") + Stt(dieuhuongDTO.so_thu_tu.Value);

            var id_dh = entity.id;
            var list = _dbContext.dieu_huong.Where(x => x.muc_luc.StartsWith(dieuhuongDTO.muc_luc) && x.id != id_dh);
            if (list.Any())
            {
        ...
        public List<TreeDieuHuongDTO> GetTreeDieuHuong()
        {
            try
            {
                ...
                if (!currentUser.super_admin)
                {
                    ...
                    {
                        cfg.AddProfile<BaseProfile>();
                        cfg.CreateMap<dieu_huong, TreeDieuHuongDTO>()
                        .ForMember(x => x.value, otp => otp.MapFrom(z => z.id))
                        .ForMember(x => x.label, otp => otp.MapFrom(z => z.ten))
                        .ForMember(x => x.is_quan_tri, otp => otp.MapFrom(z => z.is_quan_tri))
                        ...
                        .ForMember(x => x.children, otp => otp.MapFrom(z => z.ds_dieu_huong_cap_duoi.Where(x => x.is_router == false && x.ds_nhom_nguoi_dung.Any(y => y.nhom_nguoi_dung.ds_nguoi_dung.Any(t => t.nguoi_dung_id == currentUser.id))).OrderBy(x => x.so_thu_tu)));
                    });
                    var mapper = mapperCfg.CreateMapper();
                    mapperCfg.AssertConfigurationIsValid();

                    var ltsDieuHuong = _repo.Where(x => x.cap_dieu_huong == 1 && x.is_router == false && x.ds_nhom_nguoi_dung.Any(y => y.nhom_nguoi_dung.ds_nguoi_dung.Any(t => t.nguoi_dung_id == currentUser.id))).OrderBy(x => x.so_thu_tu).ToList();
                    if (ltsDieuHuong.Any())
                    {
                        tree = mapper.Map<List<dieu_huong>, List<TreeDieuHuongDTO>>(ltsDieuHuong);
                    }
                }
                else
                {
                    var mapperCfg = new MapperConfiguration(cfg =>
                    {
                        ...
                        .ForMember(x => x.name, otp => otp.MapFrom(z => z.ten))
                        .ForMember(x => x.icon, otp => otp.MapFrom(z => z.icon))
                        .ForMember(x => x.children, otp => otp.MapFrom(z => z.ds_dieu_huong_cap_duoi.Where(t => t.is_router == false).OrderBy(x => x.so_thu_tu)));
                    });
                    var mapper = mapperCfg.CreateMapper();
                    mapperCfg.AssertConfigurationIsValid();
                    var ltsDieuHuong = _repo.Where(x => x.cap_dieu_huong == 1 && x.is_router == false).OrderBy(x => x.so_thu_tu).ToList();
                    if (ltsDieuHuong.Any())
                    {
                        tree = mapper.Map<List<dieu_huong>, List<TreeDieuHuongDTO>>(ltsDieuHuong);
                    }
        ...
        public PhanLoaiDieuHuongDTO PhanLoaiDieuHuong()
        {
            try
            {
                PhanLoaiDieuHuongDTO res = new PhanLoaiDieuHuongDTO();
                List<TreeDieuHuongDTO> tree = GetTreeDieuHuong();
                res.lts_quan_tri = tree.Where(x => x.is_quan_tri == true).ToList();
                res.lts_nghiep_vu = tree.Where(x => x.is_quan_tri == false).ToList();
                return res;
            }
            catch (Exception)
            {
        ...
        public int? GetStt(Guid? dieu_huong_cap_tren_id)     
        {
            try
            {
                var max = _dbContext.dieu_huong.Where(x => x.dieu_huong_cap_tren_id == dieu_huong_cap_tren_id).Select(y => y.so_thu_tu).Max() + 1; 
                if (max != null)
                {
                    return max;
                }
        ...
        public List<dieu_huong> RecursiveUpdateEntityDieuHuong(dieu_huong entity, ItemDieuHuong parent)
        {
        ...
        public List<DieuHuongDTO> GetFlatDieuHuong()
        {
            try
            {
                ...
                var ltsDieuHuong = _repo.Where(x => x.super_admin == false).OrderBy(x => x.muc_luc).ToList();
                if (ltsDieuHuong.Any())
                {
                    res = _mapper.Map<List<dieu_huong>, List<DieuHuongDTO>>(ltsDieuHuong);
                }
        ...
        public void SttOrder(string stt_order_cha,List<TreeDieuHuongDTO> treeDieuHuongs)
        {
            try
            {
                foreach (var item in treeDieuHuongs)
                {
                    ...
                    var dh = _dbContext.dieu_huong.Find(item.value);
                    dh.stt_order = item.stt_order;
                    _dbContext.dieu_huong.Update(dh);
