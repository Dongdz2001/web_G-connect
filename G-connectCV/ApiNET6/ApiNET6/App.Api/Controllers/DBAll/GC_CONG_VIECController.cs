...
namespace App.Api.Controllers.DBAll
{
    ...
    /// GC_CONG_VIEC
    /// </summary>
    [Route("api/link-GC_CONG_VIEC")]
    [AuthorizeApi]
    [ApiController]
    public class GC_CONG_VIECController : ControllerBase
    {
        private readonly GC_CONG_VIECIService _serviceGC_CONG_VIEC;
        private readonly INhatKyHeThongService _nhatkyhethongservice;
        public GC_CONG_VIECController(GC_CONG_VIECIService serviceGC_CONG_VIEC,
            INhatKyHeThongService nhatkyhethongservice)
        {
            _serviceGC_CONG_VIEC = serviceGC_CONG_VIEC;
        ...
        public async Task<ActionResult> Get(int page = 1, int page_size = 0, string sort = null, string filter = null, string search = null)
        {
            try
            {
                var result = await _serviceGC_CONG_VIEC.GetManyAsync<GC_CONG_VIECDTO>(page, page_size, sort, filter, search);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> GetAll(string sort = null, string filter = null, string search = null)
        {
            try
            {
                var result = await _serviceGC_CONG_VIEC.GetManyAsync<GC_CONG_VIECDTO>(1, 1000000, sort, filter, search);
        ...
        public async Task<ActionResult> GetSelect(string sort = null, string filter = null, string search = null)
        {
            try
            {
                var result = await _serviceGC_CONG_VIEC.GetManyAsync<GC_CONG_VIECSelectDTO>(1, 1000000, sort, filter, search);
                return Ok(result.data);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> GetById(Guid id)
        {
            try
            {
                var result = await _serviceGC_CONG_VIEC.GetByIdAsync<GC_CONG_VIECDTO>(id);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [Route("create")]
        public async Task<ActionResult> Create([FromBody] GC_CONG_VIECDTO obj)
        {
            try
            {
                var result = await _serviceGC_CONG_VIEC.CreateAsync<GC_CONG_VIECDTO>(obj);
                ...
                try
                {
                    if (result != null)
                    {
                        ...
                        nkht.bang = "GC_CONG_VIEC";
                        nkht.ban_ghi_id = (result.id);
                        nkht.duong_dan = "/link-GC_CONG_VIEC-view/" + result.id.ToString();
                        nkht.hanh_dong = "208";
        ...
        public async Task<ActionResult> Update([FromBody] GC_CONG_VIECDTO obj, Guid id)
        {
            try
            {
                var result = await _serviceGC_CONG_VIEC.UpdateAsync<GC_CONG_VIECDTO>(obj, id);
        ...
        public async Task<ActionResult> Delete(Guid id)
        {
            try
            {
                var result = await _serviceGC_CONG_VIEC.DeleteAsync(id);
        ...
        public async Task<ActionResult> Deletes([FromBody] List<Guid> ids)
        {
            ...
            try
            {
                var result = await _serviceGC_CONG_VIEC.DeletesAsync(ids);
                return Ok(result);
            }
            catch (Exception ex)
            {
