        ...
        protected override IQueryable<GC_LICH_HANG_NGAY> QueryBuilder(IQueryable<GC_LICH_HANG_NGAY> query, dynamic filter, string search)
        {
            if (filter != null)
            {
                ...
                if (!string.IsNullOrEmpty(month))
                {
                    ...
                    query = query.Where(x => x.ngay_lam_viec != null && x.ngay_lam_viec.Value.Month== iMonth);
                }
                string year = filter.year + "";
                if (!string.IsNullOrEmpty(year))
                {
                    var iYear= int.Parse(year);
                    query = query.Where(x => x.ngay_lam_viec != null && x.ngay_lam_viec.Value.Year == iYear);
                }
