...
import ClientCorsOriginsFormView from 'modules/ClientCorsOrigins/ClientCorsOriginsFormView';
import ClientCorsOriginsForm from 'modules/ClientCorsOrigins/ClientCorsOriginsForm';
import { ClientCorsOriginsService  } from 'modules/ClientCorsOrigins/ClientCorsOriginsService';

dayjs.extend(utc)
const ClientCorsOriginsList = (props) => {
    const { t, i18n } = useTranslation();
    const openDialog = false;
    const fieldID = 'id';
    const fieldSort = 'id';
    const fieldSortOrder = 'ASC';
    const linkEdit = '/admin/ClientCorsOrigins-form';
    const linkView = '/admin/ClientCorsOrigins-view';
    const showAdvancedSearchButton = true;
    const codePermAdd = 'PERM_ClientCorsOrigins_ADD';
    const codePermModify = 'PERM_ClientCorsOrigins_MOD';
    const codePermDel = 'PERM_ClientCorsOrigins_DEL';
    const deleteMsg = 'Bạn có chắc chắn xoá bản ghi này không?';
    const titlePage = 'Danh sách Title of ClientCorsOrigins';
    ...
    const [listRef_ClientId, setListRef_ClientId] = useState([]);
    ...
    const loadLazyData = () => {
        ...
        return ClientCorsOriginsService.filterPage(advanceSearch).then(res => {
    ...
    const getListRef_ClientId = () => {
        return ClientCorsOriginsService.getAllRef_ClientId().then(res => {
            res.data.map(x => {
                x.Value = x.value;
                x.Text = x.label;
            });
            setListRef_ClientId(res.data);
        });
    }
    const onPage = (event) => {
    ...
    const deleteItem = async (id) => {
        await ClientCorsOriginsService.delete(id);
        loadLazyData();
    }
    const deleteItems = async (items) => {
        for(var i = 0; i < items.length; i++) {
            await ClientCorsOriginsService.delete(items[i][fieldID]);
        }
        loadLazyData();
    }
    ...
    return (
        <Card title={titlePage}>
            <Toast ref={toast} />
            <Dialog visible={showDialogView} onHide={() => setShowDialogView(false)} position="center" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '50vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <ClientCorsOriginsFormView id={selectedId} fnClose={closeDialog}></ClientCorsOriginsFormView>
                </div>
            </Dialog>
            <Dialog visible={showDialogEdit}position="center" onHide={() => setShowDialogEdit(false)} showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '70vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <ClientCorsOriginsForm id={selectedId} fnClose={closeDialog}></ClientCorsOriginsForm>
                </div>
            </Dialog>
                    ...
                    <div className="col-12 col-sm-3">
                        <div className="field">
                            <span className="p-float-label p-input-icon-right">
                                ...
                                <InputText name="origin" onChange={(e)=>{
                                    var filter = {
                                        ...aDVSParams,
                                ...
                                }} value={aDVSParams.Origin ?? ''} />
                                <label htmlFor="origin" >Title of Origin</label>
                            </span>
                        </div>
                    </div>
                    <div className="col-12 col-sm-3">
                        <div className="field">
                            <span className="p-float-label p-input-icon-right">
                                <i className="pi pi1-envelope" />
                                    <Dropdown options={listRef_ClientId} optionValue='Value' optionLabel='Text' scrollHeight={'600px'} className=''
                                    ...
                                    setLazyParams({
                                        ...
                                        showClear
                                    />
                                <label htmlFor="clientId" >Title of ClientId</label>
                            </span>
                        </div>
                    </div>
                    <div className="col-12 col-sm-3">                        
            ...
            >
                ...
                <Column field="Origin" sortable header={'Title of Origin'} body={(rowData, column) => decodeUnicode(rowData.origin)} />
                <Column field="ClientId" sortable header={'Title of ClientId'} body={(rowData, column) => decodeUnicode(rowData.ten_ClientId)} />
...
export default ClientCorsOriginsList;
