...
import GC_DM_LOAI_DU_ANFormView from 'modules/GC_DM_LOAI_DU_AN/GC_DM_LOAI_DU_ANFormView';
import GC_DM_LOAI_DU_ANForm from 'modules/GC_DM_LOAI_DU_AN/GC_DM_LOAI_DU_ANForm';
import { GC_DM_LOAI_DU_ANService  } from 'modules/GC_DM_LOAI_DU_AN/GC_DM_LOAI_DU_ANService';

dayjs.extend(utc)
const GC_DM_LOAI_DU_ANList = (props) => {
    const { t, i18n } = useTranslation();
    const openDialog = false;
    const fieldID = 'id';
    const fieldSort = 'id';
    const fieldSortOrder = 'ASC';
    const linkEdit = '/admin/GC_DM_LOAI_DU_AN-form';
    const linkView = '/admin/GC_DM_LOAI_DU_AN-view';
    const showAdvancedSearchButton = true;
    const codePermAdd = 'PERM_GC_DM_LOAI_DU_AN_ADD';
    const codePermModify = 'PERM_GC_DM_LOAI_DU_AN_MOD';
    const codePermDel = 'PERM_GC_DM_LOAI_DU_AN_DEL';
    const deleteMsg = 'Bạn có chắc chắn xoá bản ghi này không?';
    const titlePage = 'Danh sách Danh mục Loại dự án';
    ...
    const loadLazyData = () => {
        ...
        return GC_DM_LOAI_DU_ANService.filterPage(advanceSearch).then(res => {
    ...
    const deleteItem = async (id) => {
        await GC_DM_LOAI_DU_ANService.delete(id);
        loadLazyData();
    }
    const deleteItems = async (items) => {
        for(var i = 0; i < items.length; i++) {
            await GC_DM_LOAI_DU_ANService.delete(items[i][fieldID]);
        }
        loadLazyData();
    }
        ...
        <Card title={titlePage}>
            ...
            <Dialog visible={showDialogView} onHide={() => setShowDialogView(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '50vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <GC_DM_LOAI_DU_ANFormView id={selectedId} fnClose={closeDialog}></GC_DM_LOAI_DU_ANFormView>
                </div>
            </Dialog>
            <Dialog visible={showDialogEdit} onHide={() => setShowDialogEdit(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '70vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <GC_DM_LOAI_DU_ANForm id={selectedId} fnClose={closeDialog}></GC_DM_LOAI_DU_ANForm>
                </div>
            </Dialog>
                    ...
                    <div className="col-12 col-sm-2">
                        <div className="field">
                            <span className="p-float-label p-input-icon-right">
                                ...
                                <InputText name="ten_loai_du_an" onChange={(e)=>{
                                    var filter = {
                                        ...aDVSParams,
                                        ten_loai_du_an: e.target.value,
                                    };
                                ...
                                }} value={aDVSParams.ten_loai_du_an ?? ''} />
                                <label htmlFor="ten_loai_du_an" >Tên loại dự án</label>
                            </span>
                        </div>
                    </div>
            ...
            >
                ...
                <Column field="ten_loai_du_an" sortable header={'Tên loại dự án'} body={(rowData, column) => decodeUnicode(rowData.ten_loai_du_an)} />
                <Column field="stt" sortable header={'Thứ tự'} body={(rowData, column) => decodeUnicode(rowData.stt)} />
...
export default GC_DM_LOAI_DU_ANList;
