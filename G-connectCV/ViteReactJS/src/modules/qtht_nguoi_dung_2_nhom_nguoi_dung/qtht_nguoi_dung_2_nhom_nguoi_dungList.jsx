...
import qtht_nguoi_dung_2_nhom_nguoi_dungFormView from 'modules/qtht_nguoi_dung_2_nhom_nguoi_dung/qtht_nguoi_dung_2_nhom_nguoi_dungFormView';
import qtht_nguoi_dung_2_nhom_nguoi_dungForm from 'modules/qtht_nguoi_dung_2_nhom_nguoi_dung/qtht_nguoi_dung_2_nhom_nguoi_dungForm';
...
dayjs.extend(utc)
const qtht_nguoi_dung_2_nhom_nguoi_dungList = (props) => {
    const { t, i18n } = useTranslation();
    const openDialog = false;
    const fieldID = 'nguoi_dung_id';
    const fieldSort = 'nguoi_dung_id';
    const fieldSortOrder = 'ASC';
    const linkEdit = '/admin/qtht_nguoi_dung_2_nhom_nguoi_dung-form';
    const linkView = '/admin/qtht_nguoi_dung_2_nhom_nguoi_dung-view';
    const showAdvancedSearchButton = true;
    const codePermAdd = 'PERM_qtht_nguoi_dung_2_nhom_nguoi_dung_ADD';
    const codePermModify = 'PERM_qtht_nguoi_dung_2_nhom_nguoi_dung_MOD';
    const codePermDel = 'PERM_qtht_nguoi_dung_2_nhom_nguoi_dung_DEL';
    const deleteMsg = 'Bạn có chắc chắn xoá bản ghi này không?';
    const titlePage = 'Danh sách Title of qtht_nguoi_dung_2_nhom_nguoi_dung';
    ...
    const loadLazyData = () => {
        ...
        return qtht_nguoi_dung_2_nhom_nguoi_dungService.filterPage(advanceSearch).then(res => {
    ...
    const deleteItem = async (id) => {
        await qtht_nguoi_dung_2_nhom_nguoi_dungService.delete(id);
        loadLazyData();
    }
    const deleteItems = async (items) => {
        for(var i = 0; i < items.length; i++) {
            await qtht_nguoi_dung_2_nhom_nguoi_dungService.delete(items[i][fieldID]);
        }
        loadLazyData();
    }
        ...
        <Card title={titlePage}>
            ...
            <Dialog visible={showDialogView} onHide={() => setShowDialogView(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '50vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <qtht_nguoi_dung_2_nhom_nguoi_dungFormView id={selectedId} fnClose={closeDialog}></qtht_nguoi_dung_2_nhom_nguoi_dungFormView>
                </div>
            </Dialog>
            <Dialog visible={showDialogEdit} onHide={() => setShowDialogEdit(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '70vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <qtht_nguoi_dung_2_nhom_nguoi_dungForm id={selectedId} fnClose={closeDialog}></qtht_nguoi_dung_2_nhom_nguoi_dungForm>
                </div>
            </Dialog>
                            ...
                            <span className="p-float-label p-input-icon-right">
                                <i className="pi pi1-envelope" />
                                    <Dropdown options={listRef_nguoi_dung_id} optionValue='Value' optionLabel='Text' scrollHeight={'600px'} className=''
                                        value={lazyParams.nguoi_dung_id}
                                        onChange={(e)=>{
                                    var filter = {
                                        ...aDVSParams,
                                        nguoi_dung_id: e.target.value,
                                    };
                                    ...
                                    setLazyParams({
                                        ...
                                        showClear
                                    />
                                <label htmlFor="nguoi_dung_id" >Title of nguoi_dung_id</label>
                            </span>
                        </div>
                    </div>
                    <div className="col-12 col-sm-2">
                            ...
                            <span className="p-float-label p-input-icon-right">
                                <i className="pi pi1-envelope" />
                                    <Dropdown options={listRef_nhom_nguoi_dung_id} optionValue='Value' optionLabel='Text' scrollHeight={'600px'} className=''
                                        value={lazyParams.nhom_nguoi_dung_id}
                                        onChange={(e)=>{
                                    var filter = {
                                        ...aDVSParams,
                                        nhom_nguoi_dung_id: e.target.value,
                                    };
                                    setADVSParams(filter);
                                    setLazyParams({
                                        ...lazyParams,
                                        ...
                                        showClear
                                    />
                                <label htmlFor="nhom_nguoi_dung_id" >Title of nhom_nguoi_dung_id</label>
                            </span>
                        </div>
                    </div>
                    <div className="col-12 col-sm-2">                        
            ...
            >
                ...
                <Column field="nguoi_dung_id" sortable header={'Title of nguoi_dung_id'} body={(rowData, column) => decodeUnicode(rowData.ten_nguoi_dung_id)} />
                <Column field="nhom_nguoi_dung_id" sortable header={'Title of nhom_nguoi_dung_id'} body={(rowData, column) => decodeUnicode(rowData.ten_nhom_nguoi_dung_id)} />
...
export default qtht_nguoi_dung_2_nhom_nguoi_dungList;
