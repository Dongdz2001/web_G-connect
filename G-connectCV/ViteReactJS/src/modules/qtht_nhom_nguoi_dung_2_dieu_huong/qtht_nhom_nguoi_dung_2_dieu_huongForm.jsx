...
const qtht_nhom_nguoi_dung_2_dieu_huongForm = (props) => {
    ...
    const getListRef_nhom_nguoi_dung_id = () => {
        return qtht_nhom_nguoi_dung_2_dieu_huongService.getAllRef_nhom_nguoi_dung_id().then(res => {
            res.data.map(x => {
                x.Value = x.value;
                x.Text = x.label;
            });
    ...
    const getListRef_dieu_huong_id = () => {
        return qtht_nhom_nguoi_dung_2_dieu_huongService.getAllRef_dieu_huong_id().then(res => {
            res.data.map(x => {
                x.Value = x.value;
                x.Text = x.label;
            });
    ...
    return (
        <div >
            ...
            <Card title={(itemId === 'new' ? 'Thêm' : 'Sửa') + ' Title of qtht_nhom_nguoi_dung_2_dieu_huong'}>
                <form className="p-fluid" onSubmit={handleSubmit(onSubmit)}>
                    <div className='row'>
                        ...
                        <div className="col-12">
                            <div className="col-sm-12">
                                <label htmlFor="dieu_huong_id">
                                    {'Title of dieu_huong_id'} <span className='p-error'>(*)</span>
                                </label>
                                <Controller name="dieu_huong_id" control={control} rules={{ 
                                    required: {
                                        value: true,
                                        message: 'Vui lòng nhập!'
                                    }, 
                                    ...
                                    <Dropdown options={listRef_dieu_huong_id} optionValue='Value' optionLabel='Text' scrollHeight={'600px'} className=''
                                        {...field}
                                        value={field.value??''}
                                        placeholder='--Chọn--'
                                        showClear
                                ...
                                {getFormErrorMessage('dieu_huong_id', errors)}
                            </div>
                        </div>
...
export default qtht_nhom_nguoi_dung_2_dieu_huongForm;
