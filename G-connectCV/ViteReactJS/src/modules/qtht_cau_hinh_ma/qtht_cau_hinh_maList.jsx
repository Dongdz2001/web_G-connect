...
import qtht_cau_hinh_maFormView from "modules/qtht_cau_hinh_ma/qtht_cau_hinh_maFormView";
import qtht_cau_hinh_maForm from "modules/qtht_cau_hinh_ma/qtht_cau_hinh_maForm";
import { qtht_cau_hinh_maService } from "modules/qtht_cau_hinh_ma/qtht_cau_hinh_maService";

dayjs.extend(utc);
const qtht_cau_hinh_maList = (props) => {
  const { t, i18n } = useTranslation();
  const openDialog = false;
  const fieldID = "id";
  const fieldSort = "id";
  const fieldSortOrder = "ASC";
  const linkEdit = "/admin/qtht_cau_hinh_ma-form";
  const linkView = "/admin/qtht_cau_hinh_ma-view";
  const showAdvancedSearchButton = true;
  const codePermAdd = "PERM_qtht_cau_hinh_ma_ADD";
  const codePermModify = "PERM_qtht_cau_hinh_ma_MOD";
  const codePermDel = "PERM_qtht_cau_hinh_ma_DEL";
  const deleteMsg = "Bạn có chắc chắn xoá bản ghi này không?";
  const titlePage = "Danh sách Title of qtht_cau_hinh_ma";
  ...
  const loadLazyData = () => {
    ...
    return qtht_cau_hinh_maService.filterPage(advanceSearch).then((res) => {
  ...
  const deleteItem = async (id) => {
    await qtht_cau_hinh_maService.delete(id);
    loadLazyData();
  };
  const deleteItems = async (items) => {
    for (var i = 0; i < items.length; i++) {
      await qtht_cau_hinh_maService.delete(items[i][fieldID]);
    }
    loadLazyData();
  };
    ...
    <Card title={titlePage}>
            ...
            <div className="field">
              <span className="p-float-label p-input-icon-right">
                ...
                <InputText
                  ...
                  value={aDVSParams.gia_tri ?? ""}
                />
                <label htmlFor="gia_tri">Title of gia_tri</label>
              </span>
            </div>
          </div>
      ...
      >
        ...
        <Column
          ...
          sortable
          header={"Title of ma"}
          body={(rowData, column) => decodeUnicode(rowData.ma)}
        />
        <Column
          field="ten"
          sortable
        ...
        <Column
          field="gia_tri"
          sortable
          header={"Title of gia_tri"}
          body={(rowData, column) => decodeUnicode(rowData.gia_tri)}
        />
...
export default qtht_cau_hinh_maList;
