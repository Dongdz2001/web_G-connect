...
import qtht_google_configFormView from 'modules/qtht_google_config/qtht_google_configFormView';
import qtht_google_configForm from 'modules/qtht_google_config/qtht_google_configForm';
import { qtht_google_configService  } from 'modules/qtht_google_config/qtht_google_configService';

dayjs.extend(utc)
const qtht_google_configList = (props) => {
    const { t, i18n } = useTranslation();
    const openDialog = false;
    const fieldID = 'id';
    const fieldSort = 'id';
    const fieldSortOrder = 'ASC';
    const linkEdit = '/admin/qtht_google_config-form';
    const linkView = '/admin/qtht_google_config-view';
    const showAdvancedSearchButton = true;
    const codePermAdd = 'PERM_qtht_google_config_ADD';
    const codePermModify = 'PERM_qtht_google_config_MOD';
    const codePermDel = 'PERM_qtht_google_config_DEL';
    const deleteMsg = 'Bạn có chắc chắn xoá bản ghi này không?';
    const titlePage = 'Danh sách Title of qtht_google_config';
    ...
    const loadLazyData = () => {
        ...
        return qtht_google_configService.filterPage(advanceSearch).then(res => {
    ...
    const deleteItem = async (id) => {
        await qtht_google_configService.delete(id);
        loadLazyData();
    }
    const deleteItems = async (items) => {
        for(var i = 0; i < items.length; i++) {
            await qtht_google_configService.delete(items[i][fieldID]);
        }
        loadLazyData();
    }
        ...
        <Card title={titlePage}>
            ...
            <Dialog visible={showDialogView} onHide={() => setShowDialogView(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '50vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <qtht_google_configFormView id={selectedId} fnClose={closeDialog}></qtht_google_configFormView>
                </div>
            </Dialog>
            <Dialog visible={showDialogEdit} onHide={() => setShowDialogEdit(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '70vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <qtht_google_configForm id={selectedId} fnClose={closeDialog}></qtht_google_configForm>
                </div>
            </Dialog>
                ...
                <div className='row mb-2 advancedSearchBox'>
                    ...
                    <div className="col-12 col-sm-2">
                        <div className="field">
                            <span className="p-float-label p-input-icon-right">
                                <i className="pi pi1-envelope" />
                                <InputText name="refresh_token" onChange={(e)=>{
                                    var filter = {
                                        ...aDVSParams,
                                ...
                                }} value={aDVSParams.refresh_token ?? ''} />
                                <label htmlFor="refresh_token" >Title of refresh_token</label>
                            </span>
                        </div>
                    </div>
            ...
            >
                ...
                <Column field="email" sortable header={'Title of email'} body={(rowData, column) => decodeUnicode(rowData.email)} />
                <Column field="refresh_token" sortable header={'Title of refresh_token'} body={(rowData, column) => decodeUnicode(rowData.refresh_token)} />
...
export default qtht_google_configList;
