...
import qtht_nhom_nguoi_dung_2_dieu_huongFormView from "modules/qtht_nhom_nguoi_dung_2_dieu_huong/qtht_nhom_nguoi_dung_2_dieu_huongFormView";
import qtht_nhom_nguoi_dung_2_dieu_huongForm from "modules/qtht_nhom_nguoi_dung_2_dieu_huong/qtht_nhom_nguoi_dung_2_dieu_huongForm";
...
dayjs.extend(utc);
const qtht_nhom_nguoi_dung_2_dieu_huongList = (props) => {
  const { t, i18n } = useTranslation();
  const openDialog = false;
  const fieldID = "nhom_nguoi_dung_id";
  const fieldSort = "nhom_nguoi_dung_id";
  const fieldSortOrder = "ASC";
  const linkEdit = "/admin/qtht_nhom_nguoi_dung_2_dieu_huong-form";
  const linkView = "/admin/qtht_nhom_nguoi_dung_2_dieu_huong-view";
  const showAdvancedSearchButton = true;
  const codePermAdd = "PERM_qtht_nhom_nguoi_dung_2_dieu_huong_ADD";
  const codePermModify = "PERM_qtht_nhom_nguoi_dung_2_dieu_huong_MOD";
  const codePermDel = "PERM_qtht_nhom_nguoi_dung_2_dieu_huong_DEL";
  const deleteMsg = "Bạn có chắc chắn xoá bản ghi này không?";
  const titlePage = "Danh sách Title of qtht_nhom_nguoi_dung_2_dieu_huong";
  ...
  const [listRef_dieu_huong_id, setListRef_dieu_huong_id] = useState([]);
  ...
  useEffect(() => {
    ...
    getListRef_dieu_huong_id();
  }, [window.location.href]);
  const loadLazyData = () => {
    setLoading(true);
    console.log(lazyParams);
    let advanceSearch = mapPaginator(lazyParams);
    return qtht_nhom_nguoi_dung_2_dieu_huongService
  ...
  const getListRef_dieu_huong_id = () => {
    return qtht_nhom_nguoi_dung_2_dieu_huongService
      ...
      .then((res) => {
        ...
        setListRef_dieu_huong_id(res.data);
      });
  };
  const onPage = (event) => {
  ...
  const deleteItem = async (id) => {
    await qtht_nhom_nguoi_dung_2_dieu_huongService.delete(id);
    loadLazyData();
  };
  const deleteItems = async (items) => {
    for (var i = 0; i < items.length; i++) {
      await qtht_nhom_nguoi_dung_2_dieu_huongService.delete(items[i][fieldID]);
    }
    loadLazyData();
  };
    ...
    <Card title={titlePage}>
      ...
      >
        <div className="flex justify-content-center flex-column pt-6 px-3">
          <qtht_nhom_nguoi_dung_2_dieu_huongFormView
            id={selectedId}
            fnClose={closeDialog}
          ></qtht_nhom_nguoi_dung_2_dieu_huongFormView>
        </div>
      ...
      >
        <div className="flex justify-content-center flex-column pt-6 px-3">
          <qtht_nhom_nguoi_dung_2_dieu_huongForm
            id={selectedId}
            fnClose={closeDialog}
          ></qtht_nhom_nguoi_dung_2_dieu_huongForm>
        </div>
          ...
          <div className="col-12 col-sm-4">
            <div className="field">
              <span className="p-float-label p-input-icon-right">
                ...
                <Dropdown
                  options={listRef_nhom_nguoi_dung_id}
                  optionValue="Value"
                  optionLabel="Text"
                  scrollHeight={"600px"}
                  className=""
          ...
          <div className="col-12 col-sm-4">
            <div className="field">
              <span className="p-float-label p-input-icon-right">
                <i className="pi pi1-envelope" />
                <Dropdown
                  options={listRef_dieu_huong_id}
                  optionValue="Value"
                  optionLabel="Text"
                  scrollHeight={"600px"}
                  className=""
                  value={lazyParams.dieu_huong_id}
                  onChange={(e) => {
                    var filter = {
                      ...aDVSParams,
                      dieu_huong_id: e.target.value,
                    };
                    setADVSParams(filter);
                    setLazyParams({
                      ...lazyParams,
                  ...
                  showClear
                />
                <label htmlFor="dieu_huong_id">Title of dieu_huong_id</label>
              </span>
            </div>
          </div>
          <div className="col-12 col-sm-4">
      ...
      >
        ...
        <Column
          field="nhom_nguoi_dung_id"
          sortable
          header={"Title of nhom_nguoi_dung_id"}
          body={(rowData, column) =>
            decodeUnicode(rowData.ten_nhom_nguoi_dung_id)
          }
        />
        <Column
          field="dieu_huong_id"
          sortable
          header={"Title of dieu_huong_id"}
          body={(rowData, column) => decodeUnicode(rowData.ten_dieu_huong_id)}
        />
...
export default qtht_nhom_nguoi_dung_2_dieu_huongList;
