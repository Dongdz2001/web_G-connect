...
import GC_DM_TRANG_THAIFormView from 'modules/GC_DM_TRANG_THAI/GC_DM_TRANG_THAIFormView';
import GC_DM_TRANG_THAIForm from 'modules/GC_DM_TRANG_THAI/GC_DM_TRANG_THAIForm';
import { GC_DM_TRANG_THAIService  } from 'modules/GC_DM_TRANG_THAI/GC_DM_TRANG_THAIService';

dayjs.extend(utc)
const GC_DM_TRANG_THAIList = (props) => {
    const { t, i18n } = useTranslation();
    const openDialog = false;
    const fieldID = 'id';
    const fieldSort = 'id';
    const fieldSortOrder = 'ASC';
    const linkEdit = '/admin/GC_DM_TRANG_THAI-form';
    const linkView = '/admin/GC_DM_TRANG_THAI-view';
    const showAdvancedSearchButton = true;
    const codePermAdd = 'PERM_GC_DM_TRANG_THAI_ADD';
    const codePermModify = 'PERM_GC_DM_TRANG_THAI_MOD';
    const codePermDel = 'PERM_GC_DM_TRANG_THAI_DEL';
    const deleteMsg = 'Bạn có chắc chắn xoá bản ghi này không?';
    const titlePage = 'Danh sách Danh mục Trạng thái';
    ...
    const loadLazyData = () => {
        ...
        return GC_DM_TRANG_THAIService.filterPage(advanceSearch).then(res => {
    ...
    const deleteItem = async (id) => {
        await GC_DM_TRANG_THAIService.delete(id);
        loadLazyData();
    }
    const deleteItems = async (items) => {
        for(var i = 0; i < items.length; i++) {
            await GC_DM_TRANG_THAIService.delete(items[i][fieldID]);
        }
        loadLazyData();
    }
        ...
        <Card title={titlePage}>
            ...
            <Dialog visible={showDialogView} onHide={() => setShowDialogView(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '50vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <GC_DM_TRANG_THAIFormView id={selectedId} fnClose={closeDialog}></GC_DM_TRANG_THAIFormView>
                </div>
            </Dialog>
            <Dialog visible={showDialogEdit} onHide={() => setShowDialogEdit(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '70vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <GC_DM_TRANG_THAIForm id={selectedId} fnClose={closeDialog}></GC_DM_TRANG_THAIForm>
                </div>
            </Dialog>
                    ...
                    <div className="col-12 col-sm-2">
                        <div className="field">
                            <span className="p-float-label p-input-icon-right">
                                ...
                                <InputText name="ten_trang_thai" onChange={(e)=>{
                                    var filter = {
                                        ...aDVSParams,
                                        ten_trang_thai: e.target.value,
                                    };
                                ...
                                }} value={aDVSParams.ten_trang_thai ?? ''} />
            ...
            >
                ...
                <Column field="ten_trang_thai" sortable header={'Tên trạng thái'} body={(rowData, column) => decodeUnicode(rowData.ten_trang_thai)} />
...
export default GC_DM_TRANG_THAIList;
