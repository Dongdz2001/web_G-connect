...
const ClientCorsOriginsForm = (props) => {
    const { t, i18n } = useTranslation();
    let { id } = useParams();
    let itemId = props.id ?? id;
    const linkList = '/admin/ClientCorsOrigins';
    const labelSave = 'Lưu';
    const labelClose = 'Đóng';
    const itemService = ClientCorsOriginsService;
    ...
    return (
        <div >
            ...
            <Card title={(itemId === 'new' ? 'Thêm' : 'Sửa') + ' Title of ClientCorsOrigins'}>
                        ...
                        <div className="col-12">
                            <div className="col-sm-12">
                                <label htmlFor="origin">
                                    {'Title of Origin'} <span className='p-error'>(*)</span>
                                </label>
                                <Controller name="origin" control={control} rules={{ 
                                    required: {
                                        value: true,
                                ...
                                {getFormErrorMessage('origin', errors)}
                            </div>
                        </div>
                        <div className="col-12">
                            <div className="col-sm-12">
                                <label htmlFor="clientId">
                                    {'Title of ClientId'} <span className='p-error'>(*)</span>
                                </label>
                                <Controller name="clientId" control={control} rules={{ 
                                    required: {
                                        value: true,
                                        message: 'Vui lòng nhập!'
                                    }, 
                                ...
                                {getFormErrorMessage('clientId', errors)}
                            </div>
                        </div>
...
export default ClientCorsOriginsForm;
