...
import qtht_nhom_nguoi_dungFormView from 'modules/qtht_nhom_nguoi_dung/qtht_nhom_nguoi_dungFormView';
import qtht_nhom_nguoi_dungForm from 'modules/qtht_nhom_nguoi_dung/qtht_nhom_nguoi_dungForm';
import { qtht_nhom_nguoi_dungService  } from 'modules/qtht_nhom_nguoi_dung/qtht_nhom_nguoi_dungService';

dayjs.extend(utc)
const qtht_nhom_nguoi_dungList = (props) => {
    const { t, i18n } = useTranslation();
    const openDialog = false;
    const fieldID = 'id';
    const fieldSort = 'id';
    const fieldSortOrder = 'ASC';
    const linkEdit = '/admin/qtht_nhom_nguoi_dung-form';
    const linkView = '/admin/qtht_nhom_nguoi_dung-view';
    const showAdvancedSearchButton = true;
    const codePermAdd = 'PERM_qtht_nhom_nguoi_dung_ADD';
    const codePermModify = 'PERM_qtht_nhom_nguoi_dung_MOD';
    const codePermDel = 'PERM_qtht_nhom_nguoi_dung_DEL';
    const deleteMsg = 'Bạn có chắc chắn xoá bản ghi này không?';
    const titlePage = 'Danh sách Title of qtht_nhom_nguoi_dung';
    ...
    const loadLazyData = () => {
        ...
        return qtht_nhom_nguoi_dungService.filterPage(advanceSearch).then(res => {
    ...
    const deleteItem = async (id) => {
        await qtht_nhom_nguoi_dungService.delete(id);
        loadLazyData();
    }
    const deleteItems = async (items) => {
        for(var i = 0; i < items.length; i++) {
            await qtht_nhom_nguoi_dungService.delete(items[i][fieldID]);
        }
        loadLazyData();
    }
        ...
        <Card title={titlePage}>
            ...
            <Dialog visible={showDialogView} onHide={() => setShowDialogView(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '50vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <qtht_nhom_nguoi_dungFormView id={selectedId} fnClose={closeDialog}></qtht_nhom_nguoi_dungFormView>
                </div>
            </Dialog>
            <Dialog visible={showDialogEdit} onHide={() => setShowDialogEdit(false)} position="top" showHeader={false} showFooter={false} breakpoints={{ '960px': '80vw' }} style={{ width: '70vw' }}>
                <div className="flex justify-content-center flex-column pt-6 px-3">
                    <qtht_nhom_nguoi_dungForm id={selectedId} fnClose={closeDialog}></qtht_nhom_nguoi_dungForm>
                </div>
            </Dialog>
                ...
                <div className='row mb-2 advancedSearchBox'>
                    ...
                    <div className="col-12 col-sm-2">
                        <div className="field">
                            <span className="p-float-label p-input-icon-right">
                                <i className="pi pi1-envelope" />
                                <InputText name="mota" onChange={(e)=>{
                                    var filter = {
                                        ...aDVSParams,
                                        mota: e.target.value,
                                    };
                                ...
                                }} value={aDVSParams.mota ?? ''} />
                                <label htmlFor="mota" >Title of mota</label>
                            </span>
                        </div>
                    </div>
            ...
            >
                ...
                <Column field="mota" sortable header={'Title of mota'} body={(rowData, column) => decodeUnicode(rowData.mota)} />
...
export default qtht_nhom_nguoi_dungList;
